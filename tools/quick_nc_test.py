...
def test_nc_for_attack(waves, attack_type, num_samples=10):
    """Test NC for a specific attack with limited samples for speed"""
...
if __name__ == "__main__":
    ...
    print("Testing NC for each attack condition...")
    ...
    for condition in attack_conditions:
        result = test_nc_for_attack(waves, condition, num_samples=10)
        results.append(result)
        print(f"Completed {condition}")
    
    # Print results
    print("\n" + "="*60)
    print("NC RESULTS FOR EACH ATTACK CONDITION")
    print("="*60)
    print(f"{'Condition':<15} {'WM NC':<10} {'Clean NC':<10}")
    print("-"*60)
    
    for result in results:
        print(f"{result['condition']:<15} {result['wm_nc_avg']:<10.4f} {result['clean_nc_avg']:<10.4f}")
    ...
    # Save as JSON
    with open("audioseal_eval/nc_results.json", "w") as f:
        json.dump({
            'results': results,
            'summary': {
                'num_files': len(waves),
    ...
    print(f"\nResults saved to audioseal_eval/nc_results.json")
