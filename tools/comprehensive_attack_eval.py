...
def apply_attack(wave, attack_type):
    ...
    if attack_type == "Clean":
        ...
        # Speed up = resample to higher rate, then back to original
        speed_factor = 1.25
...
def evaluate_condition(waves, attack_type):
    """Evaluate a specific attack condition"""
...
if __name__ == "__main__":
    ...
    # Save as CSV
    df.to_csv("audioseal_eval/comprehensive_attack_results.csv", index=False)
    
    # Save as JSON
    with open("audioseal_eval/comprehensive_attack_results.json", "w") as f:
        json.dump({
            'results': results,
            'summary': {
                'num_files': len(waves),
    ...
    print(f"\nResults saved to:")
    print(f"- audioseal_eval/comprehensive_attack_results.csv")
    print(f"- audioseal_eval/comprehensive_attack_results.json")
